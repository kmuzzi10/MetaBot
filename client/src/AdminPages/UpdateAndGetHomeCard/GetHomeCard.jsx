...
const GetHomeCard = () => {
    ...
    const getAllCards = async () => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_API}/api/v1/service-cards/get-cards`);
            if (data?.success) {
            ...
            <div className='container-fluid m-3 p-3'>
                <div className='row'>
                    ...
                    <div className='col-lg-9 col-md-9 col-sm-8'>
                        <h1 style={{fontSize:'3rem'}} className='text-center'>All Home Cards List</h1>
                        <div className='row'>
                            {card.map(p => (
                                <div key={p._id} className='col-xl-4 col-lg-4 col-md-6 col-sm-12'>
                                    <Link to={`/dashboard-admin/update-home-card/${p._id}`} className='card-link'>
                                        <div className="card m-3" style={{ width: '100%' }}>
                                            <img src={`${process.env.REACT_APP_API}/api/v1/service-cards/card-photo/${p._id}`} className="card-img-top" alt={p.title} />
