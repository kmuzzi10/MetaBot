...
const UpdateClientCard = () => {
    ...
    useEffect(() => {
        const getSingleCard = async () => {
            try {
                const { data } = await axios.get(`${process.env.REACT_APP_API}/api/v1/service-cards/get-cards/${params.id}`);
                const { title, text, _id } = data.cardsData;
                setTitle(title);
                setText(text);
                setImageUrl(`${process.env.REACT_APP_API}/api/v1/service-cards/card-photo/${_id}
    ...
    const handleUpdate = async () => {
        try {
            ...
            const { data } = await axios.put(`${process.env.REACT_APP_API}/api/v1/service-cards/update-cards/${id}`, formData);
            if (data.success) {
    ...
    const handleDelete = async () => {
        try {
            ...
            if (confirmed) {
                const { data } = await axios.delete(`${process.env.REACT_APP_API}/api/v1/service-cards/delete-cards/${id}`);
                if (data.success) {
                    alert("Product deleted successfully");
                    navigate('/dashboard-admin/get-home-card'); // Redirect after successful deletion
                } else {
